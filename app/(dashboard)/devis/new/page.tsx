...
export default function NewQuotePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let quoteNumber = `DE-${new Date().getFullYear()}-001`
    if (lastQuote?.quote_number) {
      const lastNum = parseInt(lastQuote.quote_number.split('-')[2])
      quoteNumber = `DE-${new Date().getFullYear()}-${(lastNum + 1).toString().padStart(3, '0')}`
    }
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          ...
          <Button type="button" variant="secondary" onClick={() => router.push('/devis')}>
            Annuler
          </Button>
        </div>
      </form>
